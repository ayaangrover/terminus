...
RSpec.describe Terminus::Repositories::ModelPalette, :db do
  subject(:repository) { described_class.new }

  let(:record) { Factory[:model_palette] }
  ...
  describe "#find_by" do
    it "answers record when found by single attribute" do
      expect(repository.find_by(model_id: record.model_id).id).to eq(record.id)
    end

    it "answers record when found by multiple attributes" do
      attributes = record.to_h.slice :model_id, :palette_id
      expect(repository.find_by(**attributes).id).to eq(record.id)
    end

    it "answers nil when not found" do
      expect(repository.find_by(model_id: 13)).to be(nil)
    end

    it "answers nil for nil" do
      expect(repository.find_by(model_id: nil)).to be(nil)
    end
  end

  describe "#where" do
    it "answers record for single attribute" do
      ids = repository.where(model_id: record.model_id).map(&:id)
      expect(ids).to contain_exactly(record.id)
    end

    it "answers record for multiple attributes" do
      ids = repository.where(**record.to_h.slice(:model_id, :palette_id)).map(&:id)
      expect(ids).to contain_exactly(record.id)
    end

    it "answers empty array for unknown value" do
      expect(repository.where(model_id: 13)).to eq([])
    end

    it "answers empty array for nil" do
      expect(repository.where(model_id: nil)).to eq([])
    end
  end
end
