...
* `SESSION_INACTIVITY_LIMIT`: The amount of time, in seconds, since last activity the browser session remains active before needing to login again. This is checked _before_ `SESSION_LIFETIME_LIMIT`. Default: 1,800 seconds (30 minutes).
* `SESSION_LIFETIME_LIMIT`: The amount of time, in seconds, the browser session remains active -- regardless of activity -- before needing to login again. This is checked _after_ `SESSION_INACTIVITY_LIMIT`. Default: 86,400 seconds (1 day).
